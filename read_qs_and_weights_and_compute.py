...
score_col = 2135                            # Pick a column ie a Company
compname  = scores_df.iloc[0,score_col]
...
for i in range(len(scores_df)):
    ...
    if code in questions_df['Question_Code'].values:
        score = scores_df.iloc[i, score_col]
        try:
            score = float(score)
        except:
            continue
...
print(f"Scored company name:", compname)
print(f"ESG Score: {esg_score:.2f}")

# Pick q-scores of a firm
score_col = 125     # Pick a col ie a firm
firmname  = scores_df.iloc[0,score_col]
